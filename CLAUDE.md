...
The app's runtime data lives at `~/.printshelf/` (SQLite at `printshelf.sqlite3`, generated previews under `previews/`). README.md has the soft/full reset commands; close the app before resetting. `WEB_CONCURRENCY` env var overrides the default uvicorn worker count (default 1).
