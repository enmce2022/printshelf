...
uv run black spoolhouse
uv run isort spoolhouse
uv run ruff check spoolhouse
...
The classic `python -m venv .venv` + `pip install -e .[dev]` flow still works but uv is preferred. There is no build step beyond `uv sync` — the package is pure Python plus static assets under `spoolhouse/static/`.

The app's runtime data is portable: by default it lives in `./spoolhouse-data/` (SQLite at `spoolhouse.sqlite3`, generated previews under `previews/`, log at `spoolhouse.log`) — relative to the cwd at launch. Override with `--data-dir PATH` or the `SPOOLHOUSE_DATA_DIR` env var; resolution happens in `spoolhouse.desktop.resolve_data_dir`, where CLI > env var > cwd default. Nothing is written under `~/`. README.md has the reset commands; close the app first. Worker count defaults to 1; override with `--workers N`. With `N > 1`, `run_desktop_app` exports `SPOOLHOUSE_DATA_DIR` and starts uvicorn's `Multiprocess` supervisor in a daemon thread; each worker subprocess re-imports `spoolhouse.desktop:create_default_app` (the factory) to build its own service against the shared SQLite file. Shutdown hard-kills children via `Process.kill()` because uvicorn's `terminate_all` uses `CTRL_BREAK_EVENT` on Windows, which can't reach `multiprocessing.spawn` children.

Scan parallelism is independent of HTTP workers: `--scan-workers N` (or `SPOOLHOUSE_SCAN_WORKERS`) controls how many CPU cores render previews during a scan. Resolved via `spoolhouse.desktop._resolve_scan_workers` (CLI > env > 1) and stored on `SpoolHouseService.scan_workers`. When `> 1`, `scan_library` opens a `concurrent.futures.ProcessPoolExecutor` and dispatches `spoolhouse.preview.generate_preview` calls to it; the cheap parts (stat, `get_item_by_path`, `is_unchanged` decision) and the actual `db.upsert_item` write stay on the main scan thread, so SQLite writes remain serialized. The in-flight queue is bounded to `2 * scan_workers` to keep memory linear in worker count. Cancellation drops queued futures (`shutdown(wait=False, cancel_futures=True)`) and lets in-flight workers finish — matches the existing "Waiting for current file to finish..." UX.

## Architecture

SpoolHouse is a single-process desktop app: a FastAPI server runs on a background thread inside the same process as the pywebview window that displays its UI. There is no separate client/server deployment.

**Process model (`spoolhouse/desktop.py`)** — `run_desktop_app()` builds the `SpoolHouseService`, creates the FastAPI app, picks a free localhost port, starts uvicorn in a daemon thread, then opens a `webview` window pointing at that port. If pywebview fails to load (e.g. missing Linux backend), the same URL is opened in the system browser instead — so the FastAPI app must remain self-sufficient and not depend on the bridge being present.
...
- `preview.py` — generates and caches preview PNGs, keyed by SHA-1 of `(resolved_path, mtime)`. Strategy registry in `_STRATEGIES` (`StlStrategy`, `GcodeEmbeddedThumbStrategy`, `GcodeToolpathStrategy`). `generate_preview` walks strategies for the file's extension, taking the first one that returns a non-None result; embedded-thumb beats toolpath for G-code. Add new file types or thumbnail formats by appending a `PreviewStrategy` subclass — no edits to `generate_preview`. Render constants (`RENDER_WINDOW_SIZE`, `GCODE_MAX_SEGMENTS`) live at module top. Rendering uses `pyvista` (VTK offscreen) — `pv.OFF_SCREEN = True` is set at import time and `_get_plotter()` returns a process-local `pv.Plotter` reused across files within a worker (Plotter construction is the dominant per-file cost). Each `ProcessPoolExecutor` worker re-imports this module on first use, getting its own plotter; do not introduce shared global state across processes.
- `logging_setup.py` — single `configure_logging(data_dir)` call sets up a rotating file handler at `<data_dir>/spoolhouse.log` plus stderr, both at INFO. The `spoolhouse` logger root has `propagate=False` so it doesn't leak into uvicorn's default config. `SpoolHouseService.__init__` calls this; tests should pass a tmp dir.
...
**Frontend** — vanilla JS organized as native ES modules under `spoolhouse/static/js/` (no bundler). The entry point is `js/app.js`, loaded as `<script type="module">` from `index.html`. Layout: `state.js` (mutable app state), `api.js` (fetch helper), `utils.js` (`escapeHtml`, `formatBytes`, `buildItemQueryParams`, etc.), `ui/` (reusable components — `toast.js`, `modal.js` for confirm/prompt/message dialogs, `tag-input.js` for chip-style autocomplete), and `views/` (`browse.js`, `tags.js`, `scan.js`). Static assets are mounted at `/assets` and SQLite-cached previews at `/previews`. The UI has two top-level views (Browse / Tags) selected from the sidebar; tag-filtered item queries combine the active tag with current Search/Type/Sort filters server-side via `/api/items?tag=...`.
